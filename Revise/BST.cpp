...
void deleteNode(node* &root, int x) {
    if (root == nullptr) return;
        ...
        while (cur != nullptr && cur->val != x) {
            pre = cur;
            (cur->val > x)? cur = cur->left : cur = cur->right;
        }

        if (cur != nullptr) {
            if (cur->left == nullptr && cur->right == nullptr) {
                (pre->val > x)? pre->left = nullptr : pre->right = nullptr;
            } else
            if (cur->left != nullptr && cur->right == nullptr) {
                (pre->val > x)? pre->left = cur->left : pre->right = cur->left;
            } else
            if (cur->left == nullptr && cur->right != nullptr) {
                (pre->val > x)? pre->left = cur->right : pre->right = cur->right;
            } else {
                node* left_most_right = cur->right;
                node* pre_left_most_right = cur;

                while (left_most_right->left != nullptr) {
                    pre_left_most_right = left_most_right;
                    left_most_right = left_most_right->left;
                }

                cur->val = left_most_right->val;

                if (pre_left_most_right == cur) {
                    cur->right = left_most_right->right;
                } else {
                    (left_most_right->right == nullptr)? pre_left_most_right->left = left_most_right->right : pre_left_most_right->left = nullptr;
                }

                delete left_most_right;
                return;
